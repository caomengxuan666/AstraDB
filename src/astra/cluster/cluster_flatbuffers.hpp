 ...
 public:
  ...
  static std::vector<uint8_t> SerializeNodeState(const NodeInfo& node_info) {
    ...
    auto node_info_offset = AstraDB::Cluster::CreateNodeInfo(
      ...
      ip_offset,
      node_info.port,
      node_info.gossip_port,
      ConvertNodeRole(node_info.role),
      ConvertNodeStatus(node_info.status),
      node_info.config_epoch,
      node_info.heartbeat,
      node_info.shard_count,
      node_info.memory_used,
      node_info.keys_count,
      region_offset,
      node_info.replication_lag_ms
    );
    
    // Create cluster message
    auto message_offset = AstraDB::Cluster::CreateClusterMessage(
      builder,
      AstraDB::Cluster::MessageType_NodeStateUpdate,
      node_info.config_epoch,
      now,
      &node_id_struct,
      0,  // empty target node for broadcast
  ...
  static std::vector<uint8_t> SerializeSlotAssignment(uint16_t slot,
                                                       const NodeId& node_id,
                                                       MigrationAction action,
                                                       const NodeId& migration_source,
                                                       const NodeId& migration_target) {
    flatbuffers::FlatBufferBuilder builder;
    ...
    auto message_offset = AstraDB::Cluster::CreateClusterMessage(
      builder,
      AstraDB::Cluster::MessageType_SlotAssignment,
      0,  // sequence
      now,
      &node_id_struct,
      0,  // empty target node for broadcast
  ...
  static std::vector<uint8_t> SerializeDataMigration(const NodeId& source,
                                                       const NodeId& target,
                                                       uint16_t slot,
    ...
    for (const auto& [key, value] : key_values) {
      ...
      auto kv_offset = AstraDB::Cluster::CreateKeyValue(
        builder,
        key_offset,
        value_data,
        0,  // ttl_ms
    ...
    auto message_offset = AstraDB::Cluster::CreateClusterMessage(
      builder,
      AstraDB::Cluster::MessageType_DataMigration,
      0,  // sequence
      now,
      &source_struct,
  ...
  static std::vector<uint8_t> SerializeClusterSnapshot(
    ...
    for (const auto& node : nodes) {
      ...
      auto node_offset = AstraDB::Cluster::CreateNodeInfo(
        ...
        node.port,
        node.gossip_port,
        ConvertNodeRole(node.role),
        ConvertNodeStatus(node.status),
        node.config_epoch,
        node.heartbeat,
        node.shard_count,
        node.memory_used,
    ...
    std::vector<flatbuffers::Offset<AstraDB::Cluster::SlotAssignment>> slot_offsets;
    ...
    auto message_offset = AstraDB::Cluster::CreateClusterMessage(
      builder,
      AstraDB::Cluster::MessageType_SyncResponse,
      0,  // sequence
      now,
  ...
  static bool DeserializeMessage(const uint8_t* data, size_t size,
    ...
    out_type = static_cast<AstraDB::Cluster::MessageType>(message->message_type());
  ...
  static bool DeserializeDataMigration(const uint8_t* data, size_t size,
                                        NodeId& source,
                                        NodeId& target,
                                        uint16_t& slot,
  ...
  static size_t GetMessageSize(const uint8_t* data) {
      ...
      return size ? size : sizeof(flatbuffers::uoffset_t); // Fallback to minimum size
    }  
 private:
  ...
  static std::string ConvertFromNodeRole(AstraDB::Cluster::NodeRole role) {
    switch (role) {
      case AstraDB::Cluster::NodeRole_Master: return "master";
      case AstraDB::Cluster::NodeRole_Replica: return "replica";
      case AstraDB::Cluster::NodeRole_Arbiter: return "arbiter";
      default: return "master";
    }
  }
  
  // Helper: Convert NodeStatus
  static AstraDB::Cluster::NodeStatus ConvertNodeStatus(libgossip::node_status status) {
    switch (status) {
      case libgossip::node_status::online: return AstraDB::Cluster::NodeStatus_Online;
      case libgossip::node_status::suspect: return AstraDB::Cluster::NodeStatus_Suspect;
      case libgossip::node_status::failed: return AstraDB::Cluster::NodeStatus_Failed;
      case libgossip::node_status::joining: return AstraDB::Cluster::NodeStatus_Online; // Map joining to online
      case libgossip::node_status::unknown: return AstraDB::Cluster::NodeStatus_Failed; // Map unknown to failed
      default: return AstraDB::Cluster::NodeStatus_Online;
    }
  }
  
  // Helper: Convert from NodeStatus
  static libgossip::node_status ConvertFromNodeStatus(AstraDB::Cluster::NodeStatus status) {
    switch (status) {
      case AstraDB::Cluster::NodeStatus_Online: return libgossip::node_status::online;
      case AstraDB::Cluster::NodeStatus_Suspect: return libgossip::node_status::suspect;
      case AstraDB::Cluster::NodeStatus_Failed: return libgossip::node_status::failed;
      case AstraDB::Cluster::NodeStatus_Leaving: return libgossip::node_status::failed; // Map leaving to failed
      default: return libgossip::node_status::online;
    }
  }
  
  // Helper: Convert MigrationAction
  static AstraDB::Cluster::MigrationAction ConvertMigrationAction(MigrationAction action) {
    switch (action) {
      case MigrationAction::kImporting: return AstraDB::Cluster::MigrationAction_Importing;
      case MigrationAction::kMigrating: return AstraDB::Cluster::MigrationAction_Migrating;
      case MigrationAction::kStable: return AstraDB::Cluster::MigrationAction_Stable;
      default: return AstraDB::Cluster::MigrationAction_Stable;
    }
  }
};
