...
class HashSlotCalculator {
 public:
  ...
  static HashSlot Calculate(absl::string_view key) noexcept {
    return static_cast<HashSlot>(CRC16(key.data(), key.size()) %
                                 kHashSlotCount);
  }
 ...
 private:
  ...
  static uint16_t CRC16(const char* data, size_t len) noexcept {
    static constexpr uint16_t crc16_table[256] = {
        ...
        0x28a3, 0x5b7d, 0x4b5c, 0x7b3f, 0x6b1e, 0x1bf9, 0x0bd8, 0x3bbb, 0x2b9a,
        0xad07, 0xbd26, 0x8d45, 0x9d64, 0xed83, 0xfda2, 0xcde1, 0xddc0, 0xad6f,
        0xbd4e, 0x8d29, 0x9d08, 0xedef, 0xfdce, 0xcda9, 0xdd88, 0x6966, 0x7947,
        0x4924, 0x5905, 0x29e2, 0x39c3, 0x09a0, 0x1981, 0x9b5e, 0x8b7f, 0xbb1c,
        0xab3d, 0xdbda, 0xcbfb, 0xfb98, 0xebb9, 0x6a56, 0x7a77, 0x4a14, 0x5a35,
        0x2ad2, 0x3af3, 0x0a90, 0x1ab1,
    };

    uint16_t crc = 0;
    for (size_t i = 0; i < len; ++i) {
      crc = static_cast<uint16_t>(
          (crc << 8) ^ crc16_table[(crc >> 8) ^ static_cast<uint8_t>(data[i])]);
    }
    return crc;
  }
};
...
class ShardManager {
 public:
  ...
  bool Init(uint32_t shard_count, const NodeId& self_id) noexcept {
    ...
    ASTRADB_LOG_INFO(
        "ShardManager initialized: {} shards, {} slots each (approximately)",
  ...
  // Get shard count
  uint32_t GetShardCount() const noexcept { return shard_count_; }
  ...
  bool AddReplica(ShardId shard, const NodeId& replica) noexcept {
    ...
    if (std::find(replicas.begin(), replicas.end(), replica) ==
        replicas.end()) {
