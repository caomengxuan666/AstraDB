...
enum class ClusterRole { kMaster, kSlave, kNone };
...
class ClusterManager {
 public:
  ...
  // Initialize cluster
  bool Init(const std::string& node_id, const std::string& ip, uint16_t port,
            uint16_t bus_port) noexcept {
    node_id_ = node_id;
    ip_ = ip;
    port_ = port;
    bus_port_ = bus_port;
    ...
    ASTRADB_LOG_INFO("Cluster initialized: {}:{}:{} (ID: {})", ip, port,
                     bus_port, node_id);
    return true;
  }
  ...
  bool AddNode(const ClusterNode& node) noexcept {
    ...
    nodes_[node.id] = node;
    ASTRADB_LOG_INFO("Added node to cluster: {}:{} (ID: {})", node.ip,
                     node.port, node.id);
    return true;
  }
  ...
  // Get node by ID
  std::optional<ClusterNode> GetNode(
  ...
  bool AssignSlots(const std::string& node_id,
                   const std::vector<uint16_t>& slots) noexcept {
    absl::MutexLock lock(&mutex_);
    auto it = nodes_.find(node_id);
    if (it == nodes_.end()) {
      return false;
