...
class Server {
 public:
  ...
  cluster::ShardManager* GetClusterShardManager() {
    return cluster_shard_manager_.get();
  }
  ...
  cluster::GossipManager* GetGossipManagerMutable() {
    return gossip_manager_.get();
  }
  ...
  const absl::flat_hash_map<uint64_t, std::weak_ptr<network::Connection>>&
 ...
 private:
  ...
  asio::awaitable<void> HandleCommandAsync(
  ...
  absl::flat_hash_map<uint64_t, std::weak_ptr<network::Connection>>
      connections_;
  ...
  void RegisterPubSubConnection(uint64_t conn_id,
                                std::shared_ptr<network::Connection> conn);
