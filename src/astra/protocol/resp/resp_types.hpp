...
class RespValue {
 public:
  ...
  explicit RespValue(int64_t num) : type_(RespType::kInteger), value_(num) {}

  explicit RespValue(double num) : type_(RespType::kDouble), value_(num) {}

  explicit RespValue(bool b) : type_(RespType::kBoolean), value_(b) {}
  ...
  const absl::flat_hash_map<std::string, RespValue>& AsMap() const {
    if (!std::holds_alternative<absl::flat_hash_map<std::string, RespValue>>(
            value_)) {
 ...
 private:
  ...
  absl::variant<std::monostate, std::string, int64_t, double, bool,
                ...
                absl::flat_hash_map<std::string, RespValue>>
      value_;
};
