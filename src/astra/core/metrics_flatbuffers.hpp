...
public:
  ...
  static std::vector<uint8_t> SerializeCounterMetric(
    ...
    for (const auto& [label_name, label_value] : labels) {
      ...
      label_offsets.push_back(AstraDB::Metrics::CreateMetricLabel(builder, name_offset, value_offset));
    }
  ...
  // Serialize metrics batch - simplified version that just passes through the raw data
  static std::vector<uint8_t> SerializeMetricsBatch(
    ...
    for (const auto& metric_data : metrics_data) {
      if (!metric_data.empty()) {
        auto metric = flatbuffers::GetRoot<AstraDB::Metrics::Metric>(metric_data.data());
        if (metric) {
          ...
          if (metric_type == AstraDB::Metrics::MetricType_Counter && metric->counter()) {
            // Re-create counter metric
            auto counter = metric->counter();
            auto name_offset = builder.CreateString(counter->name() ? counter->name()->str() : "");
            auto help_offset = builder.CreateString(counter->help() ? counter->help()->str() : "");
            ...
            if (counter->labels()) {
              for (unsigned i = 0; i < counter->labels()->size(); i++) {
                ...
                auto label_name = builder.CreateString(label->name() ? label->name()->str() : "");
                auto label_value = builder.CreateString(label->value() ? label->value()->str() : "");
                label_offsets.push_back(AstraDB::Metrics::CreateMetricLabel(builder, label_name, label_value));
              }
            }
            auto labels_offset = builder.CreateVector(label_offsets);
          ...
          } else if (metric_type == AstraDB::Metrics::MetricType_Gauge && metric->gauge()) {
            // Re-create gauge metric
            auto gauge = metric->gauge();
            auto name_offset = builder.CreateString(gauge->name() ? gauge->name()->str() : "");
            auto help_offset = builder.CreateString(gauge->help() ? gauge->help()->str() : "");
  ...
  static bool DeserializeMetricsBatch(
    ...
    auto batch = flatbuffers::GetRoot<AstraDB::Metrics::MetricsBatch>(data.data());
    ...
    for (unsigned i = 0; i < batch->metrics()->size(); i++) {
      ...
      if (metric->metric_type() == AstraDB::Metrics::MetricType_Counter && metric->counter()) {
        // Re-create counter
        auto counter = metric->counter();
        auto name_offset = builder.CreateString(counter->name() ? counter->name()->str() : "");
        auto help_offset = builder.CreateString(counter->help() ? counter->help()->str() : "");
      ...
      } else if (metric->metric_type() == AstraDB::Metrics::MetricType_Gauge && metric->gauge()) {
        // Re-create gauge
        auto gauge = metric->gauge();
        auto name_offset = builder.CreateString(gauge->name() ? gauge->name()->str() : "");
        auto help_offset = builder.CreateString(gauge->help() ? gauge->help()->str() : "");
      ...
      if (counter_offset.o != 0 || gauge_offset.o != 0) {
        ...
        metrics_output.emplace_back(builder.GetBufferPointer(),
                                    builder.GetBufferPointer() + builder.GetSize());
      }
    }
  ...
  static bool DeserializeMetric(
    ...
    if (type_out == AstraDB::Metrics::MetricType_Counter && metric->counter()) {
      ...
      return true;
    } else if (type_out == AstraDB::Metrics::MetricType_Gauge && metric->gauge()) {
      auto gauge = metric->gauge();
