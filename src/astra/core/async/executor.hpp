...
inline Executor::Executor(size_t num_threads)
    : io_context_(1) {
  if (num_threads == 0) {
      ...
      [name](std::exception_ptr e) {
        if (e) {
          try {
            ...
            ASTRADB_LOG_ERROR("Coroutine '{}' failed: {}", name ? name : "unnamed", ex.what());
          }
        }
      });
}
...
inline size_t Executor::GetThreadIndex() const {
  ...
  return std::hash<std::thread::id>{}(std::this_thread::get_id()) % threads_.size();
}
...
inline void Executor::WorkerThread(size_t thread_index) {
  ...
  try {
    ...
    ASTRADB_LOG_ERROR("Worker thread {} exception: {}", thread_index, ex.what());
  }
