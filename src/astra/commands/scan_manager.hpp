...
class ScanStateManager {
 public:
  ...
  std::pair<uint64_t, std::vector<std::string>> GetNextBatch(uint64_t cursor,
                                                             size_t count) {
    absl::MutexLock lock(&mutex_);
    ...
    // Get next batch
    size_t end_idx =
        std::min(state->current_position + count, state->all_keys.size());
    ...
    uint64_t new_cursor =
        (state->current_position < state->all_keys.size()) ? state->scan_id : 0;
