...
constexpr size_t kHllBytes =
    (kHllRegisters * kHllBitsPerRegister) / 8;  // 12288 bytes
...
static uint64_t MurmurHash64(const std::string& key) {
  ...
  const uint8_t* tail =
      reinterpret_cast<const uint8_t*>(key.data() + blocks * 8);
  uint64_t k = 0;
  switch (key.size() & 7) {
    case 7:
...
static inline void SetRegister(std::string& hll_data, size_t idx,
                               uint8_t value) {
  size_t bit_pos = idx * kHllBitsPerRegister;
  size_t byte_pos = bit_pos / 8;
  size_t bit_offset = bit_pos % 8;
...
static uint64_t EstimateCardinality(const std::string& hll_data) {
  ...
  if (estimate <= 2.5 * kHllRegisters && zero_count > 0) {
    estimate =
        kHllRegisters * log(static_cast<double>(kHllRegisters) / zero_count);
...
CommandResult HandlePfDebug(const protocol::Command& command,
                            CommandContext* context) {
  if (command.ArgCount() < 2) {
    return CommandResult(false,
  ...
  if (!value.has_value() || value->value.size() != kHllBytes) {
    return CommandResult(
        false, "ERR key does not contain a valid HyperLogLog structure");
  }
  ...
  if (subcommand == "GETREG") {
    ...
    if (command.ArgCount() != 3) {
      return CommandResult(
          false, "ERR wrong number of arguments for 'PFDEBUG GETREG' command");
    }

    int64_t idx;
    if (!absl::SimpleAtoi(command[2].AsString(), &idx) || idx < 0 ||
        idx >= static_cast<int64_t>(kHllRegisters)) {
    ...
    return CommandResult(false,
                         "ERR unknown PFDEBUG subcommand '" + subcommand + "'");
  }
}

// PFSELFTEST - Test HyperLogLog implementation
CommandResult HandlePfSelfTest(const protocol::Command& command,
                               CommandContext* context) {
  ...
  if (empty_estimate != 0) {
    return CommandResult(false,
                         "ERR PFSELFTEST failed: empty HLL should estimate 0");
  }

  // Test 2: HLL with 1000 elements should estimate ~1000 (with some error
  // margin)
...
ASTRADB_REGISTER_COMMAND(PFADD, -2, "write", RoutingStrategy::kByFirstKey,
                         HandlePfAdd);
ASTRADB_REGISTER_COMMAND(PFCOUNT, -2, "readonly", RoutingStrategy::kByFirstKey,
                         HandlePfCount);
ASTRADB_REGISTER_COMMAND(PFMERGE, -2, "write", RoutingStrategy::kByFirstKey,
                         HandlePfMerge);
ASTRADB_REGISTER_COMMAND(PFDEBUG, -3, "readonly", RoutingStrategy::kByFirstKey,
                         HandlePfDebug);
ASTRADB_REGISTER_COMMAND(PFSELFTEST, 1, "readonly", RoutingStrategy::kNone,
                         HandlePfSelfTest);
