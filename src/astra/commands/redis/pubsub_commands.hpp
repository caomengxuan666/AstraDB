...
class PubSubManager {
 public:
  ...
  virtual std::vector<std::string> GetActiveChannels(
      const std::string& pattern = "") const = 0;

  // Get subscriber count for a specific channel
  virtual size_t GetChannelSubscriberCount(
