...
void ApplyTomlConfig(const toml::table& data, ServerConfig& config) {
  bool storage_redis_mode_seen = false;
  ...
  // Legacy RocksDB for cold data storage
  if (data["rocksdb"]) {
  ...
  // ACL
  if (data["acl"]) {
    auto acl = *data["acl"].as_table();
    config.acl.enabled = acl["enabled"].value_or<bool>(false);
    config.acl.default_user =
        acl["default_user"].value_or<std::string>("default");
    config.acl.default_password =
        acl["default_password"].value_or<std::string>("");
  }
  ...
  if (data["metrics"]) {
    ...
    if (metrics["stats_frequency_seconds"]) {
      config.metrics.stats_frequency_seconds =
          metrics["stats_frequency_seconds"].value_or<int>(10);
    }
  }

  if (storage_redis_mode_seen &&
      config.storage.mode == StorageMode::kRedis) {
    config.rdb.enabled = config.storage.redis_mode.rdb_enabled;
    config.rdb.path = config.storage.redis_mode.rdb_path;
    config.rdb.auto_save = config.storage.redis_mode.rdb_auto_save;
    config.rdb.save_interval = config.storage.redis_mode.rdb_save_interval;

    config.aof.enabled = config.storage.redis_mode.aof_enabled;
    config.aof.path = config.storage.redis_mode.aof_path;
    config.aof.sync_everysec =
        config.storage.redis_mode.aof_sync_everysec;
  }
}
