...
ASTRABI_FORCEINLINE bool CaseInsensitiveEquals_SSE2(const char* a,
  ...
  for (size_t i = 0; i + vec_size <= len; i += vec_size) {
    ...
    __m128i va_lower =
        _mm_or_si128(va, _mm_and_si128(mask, _mm_set1_epi8(0x20)));

    mask = _mm_cmpgt_epi8(vb, _mm_set1_epi8('Z'));
    __m128i vb_lower =
        _mm_or_si128(vb, _mm_and_si128(mask, _mm_set1_epi8(0x20)));
...
#if defined(ASTRABI_HAS_AVX2)
ASTRABI_FORCEINLINE int MemCompare_AVX2(const void* a, const void* b,
                                        size_t len) {
  const size_t vec_size = 32;

  for (size_t i = 0; i + vec_size <= len; i += vec_size) {
    __m256i va = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(static_cast<const char*>(a) + i));
    __m256i vb = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(static_cast<const char*>(b) + i));
...
#if defined(ASTRABI_HAS_SSE2)
ASTRABI_FORCEINLINE int MemCompare_SSE2(const void* a, const void* b,
                                        size_t len) {
  const size_t vec_size = 16;

  for (size_t i = 0; i + vec_size <= len; i += vec_size) {
    __m128i va = _mm_loadu_si128(
        reinterpret_cast<const __m128i*>(static_cast<const char*>(a) + i));
    __m128i vb = _mm_loadu_si128(
        reinterpret_cast<const __m128i*>(static_cast<const char*>(b) + i));
