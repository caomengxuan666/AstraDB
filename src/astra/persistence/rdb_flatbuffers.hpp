...
class RdbFlatbuffersSerializer {
 public:
  ...
  static std::vector<uint8_t> SerializeDatabase(int db_index,
                                                const commands::Database* db) {
    if (!db) {
      return {};
    }
    ...
    for (const auto& [key, value] : /* db->GetAllStrings() */ std::vector<
             std::pair<std::string, commands::StringValue>>()) {
      ...
      auto value_data = builder.CreateVector(
          ...
          value.value.size());
      auto string_value_offset =
          AstraDB::RDB::CreateStringValue(builder, value_data);

      auto kv_offset = AstraDB::RDB::CreateKeyValue(
          builder, key_offset, AstraDB::RDB::ValueType_String, now,
          0,  // ttl_ms
          AstraDB::RDB::Value_StringValue, string_value_offset.Union());
    ...
    auto database_offset = AstraDB::RDB::CreateDatabase(
        builder, db_index, key_values_offset, db->DbSize());
  ...
  static std::vector<uint8_t> SerializeSnapshot(
      const std::vector<std::pair<int, const commands::Database*>>& databases) {
    flatbuffers::FlatBufferBuilder builder;
    ...
    auto git_commit_offset =
        builder.CreateString(base::kVersion.git_commit_short);

    auto now =
        absl::GetCurrentTimeNanos() / 1000000;  // Convert to milliseconds

    auto header_offset =
        AstraDB::RDB::CreateHeader(builder, magic_offset, version_offset, now,
                                   git_branch_offset, git_commit_offset);
    ...
    for (const auto& [db_index, db] : databases) {
      ...
      auto db_offset = AstraDB::RDB::CreateDatabase(
          builder, db_obj->index(), db_obj->key_values(), db_obj->size());
    ...
    auto snapshot_offset = AstraDB::RDB::CreateSnapshot(
        builder, header_offset, databases_offset, total_keys,
  ...
  static bool DeserializeDatabase(const uint8_t* data, size_t size,
    ...
    for (const auto* kv : *db_obj->key_values()) {
      ...
      switch (kv->value_type()) {
        ...
        default:
          ASTRADB_LOG_WARN("Unsupported value type: {}",
                           static_cast<int>(kv->value_type()));
          break;
      }
    }
  ...
  static bool DeserializeSnapshot(
