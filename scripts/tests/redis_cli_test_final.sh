#!/bin/bash
# AstraDB Redis-CLI Test Script - Final Fixed Version
...
# Helper function to compare multi-line output
compare_output() {
    local actual="$1"
    local expected="$2"
    
    # Convert expected newlines to actual newlines
    local expected_normalized=$(echo "$expected" | sed 's/\\n/\n/g')
    local actual_normalized=$(echo "$actual" | sed 's/\\n/\n/g')
...
test_command() {
    ...
    if [[ "$expected" == *"\\n"* ]]; then
        if compare_output "$result" "$expected"; then
            echo -e "${GREEN}✓ PASS${NC}"
            PASSED_TESTS=$((PASSED_TESTS + 1))
        else
            echo -e "${RED}✗ FAIL${NC}"
            echo "  Expected (normalized): '$(echo "$expected" | sed 's/\\n/\n/g')'"
            echo "  Actual (normalized): '$(echo "$result" | sed 's/\\n/\n/g')'"
...
test_command "BLPOP (immediate)" "timeout 3 redis-cli BLPOP blocklist 1" "blocklist\nitem1"
test_command "BRPOP (empty)" "timeout 3 redis-cli BRPOP blocklist 1" "(nil)"
test_command "LPUSH before BRPOP" "LPUSH blocklist item2" "1"
test_command "BRPOP (immediate)" "timeout 3 redis-cli BRPOP blocklist 1" "blocklist\nitem2"
...
test_command "COMMAND DOCS" "COMMAND DOCS GET | head -1" "GET"  # COMMAND DOCS returns command documentation
