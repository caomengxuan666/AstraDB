...
TEST_F(AofFlatbuffersTest, HandleEmptyData) {
  ...
  bool success =
      AofFlatbuffersSerializer::DeserializeCommand(nullptr, 0, type, key);
  EXPECT_FALSE(success);

  std::vector<uint8_t> empty_data;
  success = AofFlatbuffersSerializer::DeserializeCommand(
      empty_data.data(), empty_data.size(), type, key);
  EXPECT_FALSE(success);
}
...
TEST_F(AofFlatbuffersTest, HandleInvalidData) {
  ...
  bool success = AofFlatbuffersSerializer::DeserializeCommand(
      invalid_data.data(), invalid_data.size(), type, key);
  EXPECT_FALSE(success);
}
