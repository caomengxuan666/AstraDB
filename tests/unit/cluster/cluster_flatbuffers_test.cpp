...
TEST_F(ClusterFlatbuffersTest, SerializeDeserializeHeartbeat) {
  ...
  // Serialize heartbeat
  auto data = ClusterFlatbuffers::SerializeHeartbeat(test_node_id_,
                                                     config_epoch, sequence);
  ...
  bool success = ClusterFlatbuffers::DeserializeHeartbeat(
      data.data(), data.size(), deserialized_id, deserialized_epoch,
      deserialized_seq, timestamp);
  ...
  NodeId source;
  uint64_t seq;
  success = ClusterFlatbuffers::DeserializeMessage(data.data(), data.size(),
                                                   type, seq, source);
...
TEST_F(ClusterFlatbuffersTest, SerializeDeserializeDataMigration) {
  ...
  bool success = ClusterFlatbuffers::DeserializeDataMigration(
      data.data(), data.size(), source, target, deserialized_slot,
      deserialized_kvs, deserialized_batch_id, deserialized_is_last);
...
TEST_F(ClusterFlatbuffersTest, SerializeDeserializeClusterSnapshot) {
  ...
  std::vector<std::pair<uint16_t, NodeId>> slot_assignments = {
      {0, test_node_id_}, {100, test_node_id_}, {200, test_node_id_}};
...
TEST_F(ClusterFlatbuffersTest, GetMessageSize) {
  auto data =
      ClusterFlatbuffers::SerializeHeartbeat(test_node_id_, 12345, 67890);
...
TEST_F(ClusterFlatbuffersTest, HandleEmptyData) {
  ...
  bool success = ClusterFlatbuffers::DeserializeHeartbeat(nullptr, 0, id, epoch,
                                                          seq, timestamp);
