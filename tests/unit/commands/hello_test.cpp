...
CommandResult HandleHello(const astra::protocol::Command& command,
...
class TestCommandContext : public CommandContext {
 public:
  ...
  void SetProtocolVersion(int version) override { protocol_version_ = version; }
...
TEST_F(HelloCommandTest, Hello2_ArrayHasCorrectFields) {
  ...
  // Expected: server, redis, version, 7.4.1, proto, 2, id, <connection_id>,
