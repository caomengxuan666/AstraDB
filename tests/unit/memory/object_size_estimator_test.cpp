...
TEST_F(ObjectSizeEstimatorTest, EstimateStringSize) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateStringSize(test_key_, test_value_);
  EXPECT_GT(size, 0);
...
TEST_F(ObjectSizeEstimatorTest, EstimateStringSize_LargeValue) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateStringSize(test_key_, large_value);
...
TEST_F(ObjectSizeEstimatorTest, EstimateStringDelta) {
  ...
  int32_t delta =
      ObjectSizeEstimator::EstimateStringDelta(test_key_, old_value, new_value);
...
TEST_F(ObjectSizeEstimatorTest, EstimateHashSize_EmptyHash) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateHashSize(test_key_, empty_fields);
...
TEST_F(ObjectSizeEstimatorTest, EstimateListSize_EmptyList) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateListSize(test_key_, empty_elements);
...
TEST_F(ObjectSizeEstimatorTest, EstimateSetSize_EmptySet) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateSetSize(test_key_, empty_members);
...
TEST_F(ObjectSizeEstimatorTest, EstimateZSetSize_EmptyZSet) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateZSetSize(test_key_, empty_members);
...
TEST_F(ObjectSizeEstimatorTest, EstimateStreamSize) {
  std::vector<
      ...
      entries = {{"id1", {{"field1", "value1"}, {"field2", "value2"}}},
                 {"id2", {{"field3", "value3"}}}};
  ...
  std::vector<
      std::pair<std::string, std::vector<std::pair<std::string, std::string>>>>
      larger_entries = {{"id1", {{"field1", "value1"}}},
                        {"id2", {{"field2", "value2"}}},
                        {"id3", {{"field3", "value3"}}}};
  uint32_t larger_size =
      ObjectSizeEstimator::EstimateStreamSize(test_key_, larger_entries);
  EXPECT_GT(larger_size, size);
}
...
TEST_F(ObjectSizeEstimatorTest, EstimateStreamSize_EmptyStream) {
  std::vector<
      ...
      empty_entries;
  uint32_t size =
      ObjectSizeEstimator::EstimateStreamSize(test_key_, empty_entries);
...
TEST_F(ObjectSizeEstimatorTest, EstimateStreamEntrySize) {
  ...
  uint32_t size =
      ObjectSizeEstimator::EstimateStreamEntrySize("entry_id", fields);
  EXPECT_GT(size, 0);
}
...
TEST_F(ObjectSizeEstimatorTest, EstimateMetadataSize) {
  ...
  uint32_t larger_size = ObjectSizeEstimator::EstimateMetadataSize(
      "a_very_long_key_name_with_many_characters");
  EXPECT_GT(larger_size, size);
}
...
TEST_F(ObjectSizeEstimatorTest, ConsistencyAcrossDataTypes) {
  ...
  uint32_t string_size =
      ObjectSizeEstimator::EstimateStringSize(test_key_, value);
  uint32_t list_size =
      ObjectSizeEstimator::EstimateListSize(test_key_, {value});
  ...
  EXPECT_LT(max_size - min_size,
            500);  // Difference should be less than 500 bytes
}
...
TEST_F(ObjectSizeEstimatorTest, EstimateTotalSize) {
  ...
  uint32_t total_size =
      ObjectSizeEstimator::EstimateTotalSize(test_key_, data.size());
