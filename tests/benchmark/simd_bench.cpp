...
std::vector<std::string> GenerateTestStrings(size_t count, size_t avg_length,
                                             bool include_crlf = false) {
  ...
  for (auto _ : state) {
    for (const auto& str : strings) {
      benchmark::DoNotOptimize(
          astra::base::simd::FindCRLF(str.data(), str.size()));
    }
  }
...
static void BM_HashString_Small(benchmark::State& state) {
  ...
  for (auto _ : state) {
    for (const auto& str : strings) {
      benchmark::DoNotOptimize(
          astra::base::simd::HashString(str.data(), str.size()));
    }
  }
...
static void BM_HasZero_Small(benchmark::State& state) {
  ...
  for (auto _ : state) {
    for (const auto& str : strings) {
      benchmark::DoNotOptimize(
          astra::base::simd::HasZero(str.data(), str.size()));
    }
  }
