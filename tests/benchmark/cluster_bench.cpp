...
static void BM_HashSlotCalculationWithTag(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    HashSlot slot = HashSlotCalculator::CalculateWithTag(keys[idx % keys.size()]);
    benchmark::DoNotOptimize(slot);
    ++idx;
  }
...
BENCHMARK(BM_SlotToShardMapping)
    ->Arg(4)
    ->Arg(16)
    ->Arg(64)
    ->Arg(256);
...
static void BM_GossipTickOverhead(benchmark::State& state) {
  ...
  // Note: Init may fail without network binding, so we just benchmark the tick call
  // In a real scenario, this would be benchmarked with actual network setup
  
  for (auto _ : state) {
    // Simulate tick overhead (even without network)
    // This measures the baseline CPU cost of calling tick
    benchmark::DoNotOptimize(0);  // Placeholder - actual gossip tick requires Init
  }
...
static void BM_RedirectCheck(benchmark::State& state) {
  ...
  // Simulate: for each key, calculate slot, get shard, check if owner matches self
  size_t idx = 0;
  for (auto _ : state) {
